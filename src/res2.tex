...
\end{equation}
    ...
    pendiente por el valor de la constante de la gravedad 
    ($g = \SI{9.80665}{\meter\per\second\squared}$, incertidumbre despreciable).
    Los resultados de estas operaciones se encuentran resumidos en la tabla
    \ref{tab:2:caudales}, y fueron utilizados junto con los diámetros de la 
    tabla \ref{tab:datos:diametros} para construir el gráfico de la figura
    \ref{fig:2:caudales}.
...
\begin{table}[H]
    ...
    \csvreader[
        tabular = r|c,
        table head = \toprule Tapa & Caudal (\si{\kg\per\second}) \\\midrule,
